...
{
    /// <summary>
    /// PlayMaker custom action to start playing video with video player
    /// 
    ...
    [Tooltip("Start video with video player. The video player must be initialized beforehand.")]
    public class VideoPlay : FsmStateAction
    {
        ...
        public override void OnEnter()
        {
            ...
            StylyServiceForPlayMaker.Instance.VideoPlay(go, error =>
            {
                if (error != null)
                {
                    if (errorString != null)
